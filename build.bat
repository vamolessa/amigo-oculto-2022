...
if not defined vs_dir (
    for /f "tokens=*" %%d in ('"%programfiles(x86)%\Microsoft Visual Studio\Installer\vswhere.exe" -latest -requires Microsoft.VisualStudio.Component.VC.Tools.x86.x64 -property installationPath') do set vs_dir=%%d
)
if not defined vs_dir (
...
set CLI=%CLI% main.c /Feamigo-oculto.exe /link /SUBSYSTEM:windows User32.lib Gdi32.lib Winmm.lib resources.res
%CLI%
